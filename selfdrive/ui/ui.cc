...
static void update_params(UIState *s) {
  ...
  if (!scene.navi_on_boot && (frame - scene.started_frame > 2*UI_FREQ)) {
    if (Params().getBool("OpkrRunNaviOnBoot") && (Params().get("CarParams").size() > 0)) {
  ...
  if (!scene.move_to_background && (frame - scene.started_frame > 7*UI_FREQ)) {
    if (Params().getBool("OpkrRunNaviOnBoot") && Params().getBool("OpkrMapEnable") && (Params().get("CarParams").size() > 0)) {
